...
void StereoCalib(const vector<string>& imagelist, Size boardSize,
                 float squareSize, bool displayCorners, bool useCalibrated,
                 bool showRectified);

void testCalibration() {

  Calibration calibration(1280, 480, 9, 6, 0.02354, 30);
  ...
  Mat leftImg;
  Mat rightImg;
  printf("Start to calibrate stereo camera!\n");
  while (!calibration.findSufficientChessBoard()) {
    ...
    if (key == 'y') {
      calibration.findChessboard(leftImg, rightImg, true);
    }
  }
#endif
  ...
  int i = 0;
  while (!calibration.findSufficientChessBoard()) {
    char leftAddr[50], rightAddr[50];
    sprintf(leftAddr, "chessBoard_Left_%d.png", i);
    sprintf(rightAddr, "chessBoard_Right_%d.png", i);
    string left(leftAddr);
    string right(rightAddr);
    imagelist.push_back(left);
    imagelist.push_back(right);
    srcFromLeftCamera = imread(leftAddr);
    srcFromRightCamera = imread(rightAddr);
    i++;
    calibration.findChessboard(srcFromLeftCamera, srcFromRightCamera, false);
  }
#endif

  //StereoCalib(imagelist, Size(9, 6), 0.02354, false, false, false);
  printf("find sufficient chess board images!\n");
  calibration.finishFindChessboard();
  printf("finish to find chess board!\n");
  struct timeval tpstart, tpend;
  gettimeofday(&tpstart, NULL);
  calibration.singleCameraCalibration(0);
  calibration.singleCameraCalibration(1);
  calibration.stereoCameraCalibration();
  gettimeofday(&tpend, NULL);
  long timuse = 1000 * (tpend.tv_sec - tpstart.tv_sec)
      + (tpend.tv_usec - tpstart.tv_usec) / 1000;
  printf("Time of camera calibration %ld\n", timuse);
  printf("Quality check: %f\n", calibration.calibrationQualityCheck());
  calibration.printCalibrationError();
  calibration.writeCalibratedParamToFile("calibration.xml");
}
...
void StereoCalib(const vector<string>& imagelist, Size boardSize,
    ...
    for (k = 0; k < 2; k++) {
      ...
      for (int scale = 1; scale <= maxScale; scale++) {
        ...
        if (scale == 1)
          ...
          resize(img, timg, Size(), scale, scale, 0);
      ...
      if (displayCorners) {
        ...
        resize(cimg, cimg1, Size(), sf, sf, 0);
  ...
  cameraMatrix[0] = initCameraMatrix2D(objectPoints, imagePoints[0], imageSize,
                                       0);
  cameraMatrix[1] = initCameraMatrix2D(objectPoints, imagePoints[1], imageSize,
                                       0);
  Mat R, T, E, F;
